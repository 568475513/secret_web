...
func (a *AppInfo) handleConfResult(result service.ConfHubInfo) *service.AppBaseConf {
	...
	baseConf := &service.AppBaseConf{
		...
		IsPrivacyProtection: int(result.Live["is_privacy_protection"].(float64)),
...
// 获取云通信配置
func (a *AppInfo) GetCommunicationCloudInfo(identifier string, roomId string, fusing bool) map[string]string {
	conf := map[string]string{
		"user_sign":    "",
		"sdk_app_id":   "",
		"account_type": os.Getenv("AccountType"),
	}
	if !fusing && !strings.Contains(roomId, "XET#") {
