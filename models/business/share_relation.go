...
func GetAvailShareInfo(appId string, resourceType int, resourceId string, productId string, userId string) (*ShareRelation, error) {
	...
	err := db.Table("t_share_relation").Where("app_id = ? and payment_type = ? and resource_type = ? and resource_id = ? and product_id = ? and receive_user_id = ? and received_at <> null",
...
func GetHasReceivedShare(appId string, resourceType int, resourceId string, productId string, userId string) ([]*ShareRelation, error) {
	...
	err := db.Table("t_share_relation").Select("receive_user_id").Where("app_id = ? and payment_type = ? and resource_type = ? and resource_id = ? and product_id = ? and receive_user_id = ? and received_at <> null",
